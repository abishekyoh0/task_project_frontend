...
import { User, Building, Lock, Loader2, CheckCircle } from "lucide-react";
...
export default function Settings() {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(
        err.response?.data?.error?.message || "Profile update failed",
        "error",
      );
    } finally {
  ...
  const handleUpdateOrg = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(
        err.response?.data?.error?.message || "Organization update failed",
        "error",
      );
    } finally {
      setOrgLoading(false);
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(
        err.response?.data?.error?.message || "Password update failed",
        "error",
      );
    } finally {
    ...
    <div className="space-y-8 animate-in fade-in duration-200">
      ...
      <div>
        ...
        <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">
          Configure profile preferences, password security, and tenant
          configurations.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-6">
          <div className="flex items-center space-x-3 pb-4 border-b border-slate-100 dark:border-slate-800">
            ...
            <h3 className="font-bold text-slate-800 dark:text-white my-0">
              Personal Profile
            </h3>
          </div>
